...
/**
 * Solución: BFS con Cola (Queue)
 *
 * Estrategia:
 * - Usar una cola para almacenar los nodos por procesar
 * - En cada iteración, procesar todos los nodos del nivel actual
 * - Agregar los hijos de esos nodos a la cola para el siguiente nivel
...
function levelOrder(root) {
  ...
  const result = [];
  const queue = [root]; // Cola con nodos a procesar

  while (queue.length > 0) {
    const levelSize = queue.length; // Número de nodos en el nivel actual
    const currentLevel = [];

    // Procesar todos los nodos del nivel actual
    for (let i = 0; i < levelSize; i++) {
      const node = queue.shift(); // Sacar el primer nodo
      currentLevel.push(node.val); // Agregar valor al nivel actual

      // Agregar hijos a la cola para procesar en el siguiente nivel
      if (node.left !== null) {
    ...
    // Agregar el nivel completo al resultado
...
/**
 * Solución Alternativa: DFS Recursivo
 *
 * Estrategia:
 * - Usar recursión con un parámetro de profundidad
 * - Para cada nodo, agregarlo a la lista del nivel correspondiente
 * - Procesar primero izquierda, luego derecha
 *
 * @param {TreeNode} root - Raíz del árbol binario
 * @return {number[][]} - Lista de listas con valores por nivel
 */
function levelOrderDFS(root) {
  ...
  function dfs(node, level) {
    ...
    // Si no existe lista para este nivel, crearla
    if (result.length === level) {
      result.push([]);
    }

    // Agregar valor al nivel correspondiente
...
/**
 * Solución Alternativa: Level Order con Decoración (Retorna pares nodo-nivel)
 *
 * Útil si necesitas más información sobre la posición de cada nodo
 */
function levelOrderWithDepth(root) {
  ...
  const result = [];
  const queue = [[root, 0]]; // [nodo, profundidad]
