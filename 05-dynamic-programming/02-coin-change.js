...
// Patrón: Dynamic Programming - Tabulación (Unbounded Knapsack)
// ============================================
...
function coinChange(coins, amount) {
  // Inicializamos con un valor grande (infinito)
  const dp = Array(amount + 1).fill(Infinity);
  dp[0] = 0; // Base: 0 monedas para cantidad 0

  // Iteramos cada cantidad de 1 a amount
  for (let i = 1; i <= amount; i++) {
    // Probamos cada moneda disponible
    for (const coin of coins) {
      if (coin <= i) {
        // Si esta moneda cabe, vemos si mejora nuestra solución
        dp[i] = Math.min(dp[i], dp[i - coin] + 1);
      }
    }
  }

  // Si dp[amount] sigue siendo Infinity, es imposible
...
function coinChangeWithCoins(coins, amount) {
  ...
  for (let i = 1; i <= amount; i++) {
    for (const coin of coins) {
      if (coin <= i && dp[i - coin] + 1 < dp[i]) {
        dp[i] = dp[i - coin] + 1;
        parent[i] = coin; // Guardamos qué moneda usamos
      }
    }
  }
  ...
  // Reconstruimos las monedas usadas
  const usedCoins = [];
