...
/**
 * SOLUCIÓN 1: Pila Monótona Decreciente (RECOMENDADA)
 *
 * Estrategia:
 * - Usar una pila para rastrear índices en orden decreciente de temperatura
 * - Procesar desde derecha a izquierda (desde el final al inicio)
 * - Cuando encontramos un día más cálido, calculamos la diferencia
 */
function dailyTemperatures(temperatures) {
  const n = temperatures.length;
  const result = new Array(n).fill(0);
  const stack = []; // Stack de índices

  // Recorremos de derecha a izquierda
  for (let i = n - 1; i >= 0; i--) {
    // Removemos índices de temperaturas más bajas o iguales
    while (stack.length > 0 && temperatures[stack[stack.length - 1]] <= temperatures[i]) {
      stack.pop();
    }

    // Si la pila no está vacía, el tope tiene la próxima temperatura más cálida
    if (stack.length > 0) {
      result[i] = stack[stack.length - 1] - i;
    }

    // Agregamos el índice actual a la pila
    stack.push(i);
  }
...
/**
 * SOLUCIÓN 2: Enfoque Ingenuo (para comparación)
 *
 * Para cada día, busca hacia adelante hasta encontrar un día más cálido
 */
function dailyTemperaturesNaive(temperatures) {
