...
/**
 * Solución: Preorder Traversal (Recomendado)
 *
 * Estrategia:
 * SERIALIZACIÓN:
 * - Usar preorder traversal (raíz, izquierda, derecha)
 * - Agregar "null" para representar nodos faltantes
 * - Separar valores con coma
 *
 * DESERIALIZACIÓN:
 * - Parsear la cadena en array
 * - Usar índice para reconstruir recursivamente
 * - Crear nodo, procesar izquierda, procesar derecha
 *
 * El preorder es ideal porque el primer elemento siempre es la raíz
 */
class Codec {
  /**
...
/**
 * Solución Alternativa: Level Order Traversal (BFS)
 *
 * Estrategia:
 * SERIALIZACIÓN:
 * - Usar BFS para recorrer nivel a nivel
 * - Incluir "null" para todos los espacios vacíos
 * - Esto es más parecido a cómo se representa arrays en árboles
 *
 * DESERIALIZACIÓN:
 * - Reconstruir nivel a nivel
 * - Más complicado porque necesitamos mantener pares (padre, índice hijo)
 */
class CodecBFS {
  serialize(root) {
    if (root === null) {
    ...
    // Remover nulls finales para optimizar
